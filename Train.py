...
if __name__=='__main__':
	...
	train, valid = torch.utils.data.random_split(data, [40000, 10000])

	train = Loading.DataSet(train, "norm")
	valid = Loading.DataSet(valid, "eval")
	...
	for epoch in range(100):
		...
		f = open("warning.txt", "a")
		f.write("[Done] Computing train loss: {:.4f}\n".format(np.mean(train_loss)))
		f.write("[Done] Computing train scre: {:.4f}\n".format(np.mean(train_scre)))
		f.close()
		...
		f = open("warning.txt", "a")
		f.write("[Done] Computing valid loss: {:.4f}\n".format(np.mean(valid_loss)))
		f.write("[Done] Computing valid scre: {:.4f}\n".format(np.mean(valid_scre)))
		f.close()
